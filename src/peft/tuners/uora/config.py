...
class UoraConfig(PeftConfig):
    ...
    Args:
        ...
        target_modules (`Union[List[str], str]`):
            The names of the modules to apply UORA to. Only linear layers are supported.
        projection_prng_key (`int`):
            UORA PRNG init key. Used for initialising uora_A and uora_B for new models or when loading a checkpoint
            that did not include these projections. Defaults to `0`.
        save_projection (`bool`):
            Whether to save the uora_A / uora_B projections in the state dict alongside per layer lambda_b / lambda_d
        ...
        uora_dropout (`float`):
            The dropout probability for UORA layers.
        d_initial (`float`, *optional*, defaults to `0.1`):
            Initial init value for `uora_lambda_d` vector used when initializing the UORA parameters. Small values
            (<=0.1) are recommended (see Table 6c in the paper).
        fan_in_fan_out (`bool`):
            Set this to True if the layer to replace stores weight like (fan_in, fan_out). For example, gpt-2 uses
            `Conv1D` which stores weights like (fan_in, fan_out) and hence this should be set to `True`.
        bias (`str`):
            Bias type for UORA. Can be 'none', 'all' or 'uora_only'. If 'all' or 'uora_only', the corresponding biases
            will be updated during training. Be aware that this means that, even when disabling the adapters, the model
            will not produce the same output as the base model would have without adaptation.
        modules_to_save (`List[str]`):
            List of modules apart from UORA layers to be set as trainable and saved in the final checkpoint.
        init_weights (`bool`):
            Whether to initialize the weights of the UORA layers with their default initialization. Don't change this
            setting, except if you know exactly what you're doing.
        layers_to_transform (`Union[List[int],int]`):
            The layer indexes to transform, if this argument is specified, it will apply the UORA transformations on
            the layer indexes that are specified in this list. If a single integer is passed, it will apply the UORA
    ...
    r: int = field(default=256, metadata={"help": "UORA attention dimension"})

    target_modules: Optional[Union[list[str], str]] = field(
        default=None,
        metadata={
            "help": (
                "List of module names or regex expression of the module names to replace with UORA."
    ...
    projection_prng_key: int = field(
        ...
        metadata={
            "help": (
                "UORA PRNG init key. Used for initialising uora_A and uora_B for new models or when loading a "
    ...
    save_projection: bool = field(
        ...
        metadata={
            "help": (
                "Whether to save the uora_A / uora_B projections in the state dict alongside per layer lambda_b / "
    ...
    uora_dropout: float = field(default=0.0, metadata={"help": "UORA dropout"})
    d_initial: float = field(default=0.1, metadata={"help": "Initial init value for d vector."})
    fan_in_fan_out: bool = field(
        default=False,
        metadata={"help": "Set this to True if the layer to replace stores weight like (fan_in, fan_out)"},
    )
    bias: str = field(default="none", metadata={"help": "Bias type for UORA. Can be 'none', 'all' or 'uora_only'"})
    modules_to_save: Optional[list[str]] = field(
        default=None,
        metadata={
            "help": (
                "List of modules apart from UORA layers to be set as trainable and saved in the final checkpoint. For"
    ...
    init_weights: bool = field(
        ...
        metadata={
            "help": (
                "Whether to initialize the weights of the UORA layers with their default initialization. Don't change "
